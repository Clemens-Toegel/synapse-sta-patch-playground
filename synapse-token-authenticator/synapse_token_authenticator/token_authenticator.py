...
class TokenAuthenticator:
        ...
        if (custom_flow := getattr(self.config, "custom_flow", None)) is not None:
            if custom_flow.secret:
                ...
                with open(custom_flow.keyfile) as f:
                    self.custom_flow_key = jwk.JWKSet.from_json(f.read())
    ...
    async def check_oidc_auth(
        ...
        allowed_roles = ["hcp", "verifier_provider", "xt_representative", "patient"]
        ...
        # Taken from the JWT login part, as we don't have the property "localpart"
        # we check for "sub"
        token_user_id_or_localpart = introspection_resp["sub"]
        if not isinstance(token_user_id_or_localpart, str):
            logger.info("user_id isn't a string")
            return None

        # Check for localpart in "sub" property instead of "localpart"
        if token_user_id_or_localpart != user_id.localpart:
